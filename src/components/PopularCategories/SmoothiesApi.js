...
import smoothiesbg from "./images2/smoothiesbg.jpg";
 import Cards2 from "./Cards2";

export default function SmoothiesApi() {
  const [getFullData, setgetFullData] = useState([]);

  const backgroundStyle = {
    backgroundImage: `url(${smoothiesbg})`,
    ...
    const getDeliciousFood = async () => {
      try {
        const res = await axios.get(
         ...
         "https://api.edamam.com/api/recipes/v2?type=public&q=smoothies&app_id=cb9a6a22&app_key=607d5b31a3c0de4c5ae27468259004a7"
        );
