...
import breakfastbg from "./images2/breakfastbg.jpg";
 import Cards2 from "./Cards2";

export default function BreakfastApi() {
  const [getFullData, setgetFullData] = useState([]);

  const backgroundStyle = {
    backgroundImage: `url(${breakfastbg})`,
    ...
    const getDeliciousFood = async () => {
      try {
        const res = await axios.get(
         ...
         "https://api.edamam.com/api/recipes/v2?type=public&q=breakfast&app_id=cb9a6a22&app_key=607d5b31a3c0de4c5ae27468259004a7"
        );
